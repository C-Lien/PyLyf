...
def create_directory():
    ...
    if not messagebox.askyesno("Create Directory", f"Create {HOME}?"):
        sys.exit()
    (HOME / "Wallpapers").mkdir(parents=True); (HOME / "Generated").mkdir()
    root.destroy()

create_directory()

# ────────────────── CONFIG
cfg = {k: None for k in ("WALLPAPER_DIRECTORY",
...
with (HOME / "CONFIG").open() as f:
    for k,v in (l.strip().split("=",1) for l in f):
        if k.strip() in cfg: cfg[k.strip()] = v.strip().strip('"')
...
################################################################################
if ctypes.windll.user32.SystemParametersInfoW(SPI_SETWALLPAPER,
                                              0,
                                              str(generated_wallpaper),
                                              3):
